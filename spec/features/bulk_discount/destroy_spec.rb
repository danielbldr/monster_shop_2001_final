...
RSpec.describe 'As a merchant employee', type: :feature do
  it "I can delete a discount" do
    ...
    within("#discount-#{bulk_discount2.id}") do
      click_link bulk_discount2.description
    end
    ...
    expect(page).to_not have_css("#discounts-#{bulk_discount2.id}")
    expect(page).to_not have_content(bulk_discount2.description)
    expect(page).to_not have_content(bulk_discount2.minimum_items)
    expect(page).to_not have_content(bulk_discount2.percentage_off)
  end
end
